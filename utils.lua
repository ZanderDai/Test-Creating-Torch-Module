...
function zz.utils.recursiveType(param, type_str)
   if torch.type(param) == 'table' then
      for k, v in pairs(param) do
         param[k] = zz.utils.recursiveType(v, type_str)
      end
   elseif torch.isTypeOf(param, 'zz.Module') or
          torch.isTypeOf(param, 'zz.Criterion') then
      param:type(type_str)
...
function zz.utils.recursiveResizeAs(t1,t2)
   if torch.type(t2) == 'table' then
      t1 = (torch.type(t1) == 'table') and t1 or {t1}
      for key,_ in pairs(t2) do
         t1[key], t2[key] = zz.utils.recursiveResizeAs(t1[key], t2[key])
      end
...
function zz.utils.recursiveFill(t2, val)
   if torch.type(t2) == 'table' then
      for key,_ in pairs(t2) do
         t2[key] = zz.utils.recursiveFill(t2[key], val)
      end
   elseif torch.isTensor(t2) then
      t2:fill(val)
   else
...
function zz.utils.recursiveAdd(t1, val, t2)
   if not t2 then
   ...
   if torch.type(t2) == 'table' then
      t1 = (torch.type(t1) == 'table') and t1 or {t1}
      for key,_ in pairs(t2) do
         t1[key], t2[key] = zz.utils.recursiveAdd(t1[key], val, t2[key])
      end
...
function zz.utils.addSingletonDimension(t, dim)
